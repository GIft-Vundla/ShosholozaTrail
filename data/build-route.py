...
attractions=[dict(id=id,hubId=hub,name=name,recordType='attraction',lat=None,lon=None,
                  coordinateStatus='not-geocoded',sourceIds=[source],visibilityFromTrain='not-established')
             for id,hub,name,source in [
                 ('freedom-park','pretoria','Freedom Park','freedom-park'),
                 ('sol-plaatje-museum','kimberley','Sol Plaatje Educational Trust and Museum','sol-plaatje'),
                 ('karoo-fossil-trail','beaufort-west','Karoo National Park Fossil Trail','sanparks-karoo'),
                 ('lord-milner-hotel','matjiesfontein','Lord Milner Hotel','matjiesfontein-history'),
                 ('zwelethemba-heritage-route','worcester','Zwelethemba Heritage Route','worcester-heritage'),
                 ('district-six-museum','cape-town','District Six Museum','district-six')]]

def write(path,obj):
    (ROOT/path).write_text(json.dumps(obj,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
write(Path('route.geojson'),route)
write(Path('hubs.json'),dict(version=version,stations=stations,triggerZones=triggers,attractions=attractions,
